...
/**
 * Service for direct interaction with AWS Bedrock models
 * This implementation is browser-compatible and doesn't require Node.js polyfills
 */
class BedrockService {
  /**
   * Get an authenticated BedrockRuntimeClient
   * @private
   */
  async _getBedrockClient() {
    try {
      // Get AWS credentials from Amplify Auth
      const credentials = await Auth.currentCredentials();

      // Create a BedrockRuntimeClient with the credentials
      ...
      throw new Error(`Failed to create Bedrock client: ${error.message}`);
    }
  }
  ...
  async invokeModel(modelId, prompt, parameters = {}) {
    try {
      console.log(`BedrockService: Invoking model ${modelId}`);
      
      // Get a configured BedrockRuntimeClient
      const bedrockClient = await this._getBedrockClient();
      
      // Prepare model parameters based on the model type
      const modelParams = this._getModelRequestBody(modelId, prompt, parameters);
      ...
      const generatedText = this._extractGeneratedText(modelId, responseBody);
  ...
  async analyzeCandidate(candidate, jobInfo, options = {}) {
    try {
      console.log('BedrockService: Analyzing candidate profile');
      ...
      console.error('BedrockService: Error analyzing candidate:', error);
      return {
        success: false,
